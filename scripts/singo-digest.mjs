/**
 * 오늘의 신고가 — 콘텐츠 재료 만들기 (네트워크 불필요, 누적 로그만 읽는다).
 *
 * ── 왜 (2026-08-12 오너)
 * 처음엔 주간("금주의 신고가")으로 잡았는데, 돌파가 **주 1~2건**이라 카드가 빈다고 알리자
 * 오너가 **"주간 하지말고 매일 보내줘"** 로 바꿨다. 그래서 기본이 `--days 1`(하루)이다.
 * 여러 날을 묶고 싶으면 `--days 7` 로 그날 돌리면 된다 — 배관은 그대로 쓴다.
 *
 * 두 가지를 만든다:
 ...
 * 소식이 그날의 콘텐츠다. 계약일은 카드에 그대로 싣는다.
 *
 * ── 돌파는 수집기가 이미 정확히 계산해 뒀다
 * 여기서 **다시 판정하지 않는다.** 수집기가 판정 시점의 인덱스(=직전 최고가)로 계산한
 * `milestone` 을 그대로 모은다. 여기서 다시 재면 인덱스가 이미 갱신돼 있어 틀린다.
 *
 * 실행: node scripts/singo-digest.mjs [--today 2026-08-13] [--days 1]
 */
...
/** 하루면 날짜 하나, 여러 날이면 구간으로 적는다 */
const span = days <= 1 ? today : `${from} ~ ${today}`;
...
writeFileSync(join(D, "singo-digest.json"), JSON.stringify(out, null, 2) + "\n");

/* ── 텔레그램 문구 */
const T = [];
T.push(`📅 신고가 콘텐츠 재료 (${span})`);
T.push("");
if (crossed.length) {
  T.push(`🎉 돌파 ${crossed.length}건 — 컨셉A 카드 재료`);
  for (const h of crossed) T.push(`· ${h.aptNm} ${h.pyeong} ${eok(h.priceManwon)} → ${h.milestone}억 돌파`);
} else {
  T.push(`🎉 돌파 0건 — 10억 단위를 넘은 단지는 없습니다`);
}
T.push("");
if (cand.length) {
  ...
  T.push("");
  T.push(`전체 목록: docs/daily/신고가재료-${today}.md`);
} else {
  T.push(`📋 신고가 0건 — data/singo-last.md 로 수집이 돌았는지 확인하세요`);
}
writeFileSync(join(ROOT, "data/singo-digest-alert.txt"), T.join("\n") + "\n");

/* ── 오너가 고를 md */
const L = [];
L.push(`# 📅 신고가 콘텐츠 재료 — ${span}`);
L.push("");
L.push(`> 1,000세대 이상 1,147개 단지 명부 안에서, 전용 59타입(25평)·84타입(34평) 중개거래 중`);
L.push(`> **2020년 이후 최고가를 새로 쓴 것**. 직거래·해제거래 제외.`);
L.push(`> 기간은 **확인한 날(foundOn) 기준**이다 — 실거래 신고기한이 30일이라 계약일로 자르면 목록이 빈다.`);
L.push("");
L.push(`| | |`);
L.push(`|---|---|`);
L.push(`| 기간 | ${span}${days > 1 ? ` (${days}일)` : ""} |`);
L.push(`| 신고가 | ${n(week.length)}건 |`);
L.push(`| **10억 단위 돌파** | **${n(crossed.length)}건** |`);
L.push("");
L.push("---");
L.push("");
L.push(`## 컨셉 A — 돌파 단지 (${n(crossed.length)}건)`);
L.push("");
if (crossed.length) {
  ...
  L.push("10억 단위를 넘은 단지가 없습니다.");
  L.push("");
  L.push("> 10억 돌파는 실측상 **월 8~12건**입니다(2026-07 기준). 며칠씩 없는 날이 정상이고,");
  L.push("> 그런 날은 아래 컨셉 B 후보에서 골라 개별 카드로 갑니다.");
  L.push("> 며칠을 묶어 보려면 `node scripts/singo-digest.mjs --days 7` 로 돌립니다.");
}
L.push("");
L.push("---");
L.push("");
...
if (cand.length) {
  ...
  L.push("신고가가 0건입니다. `data/singo-last.md` 로 수집이 돌았는지 먼저 확인하세요.");
}
L.push("");
L.push("---");
L.push("");
...
L.push("  `data/datasets/singo-digest.json` 에 `priceManwon` 으로 있다.");
...
L.push("");
L.push(`이 문서는 \`node scripts/singo-digest.mjs --today ${today} --days ${days}\` 로 다시 만든다.`);
L.push("");

const mdPath = join(ROOT, `docs/daily/신고가재료-${today}.md`);
