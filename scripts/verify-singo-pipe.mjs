...
head("⑦ 자료가 없으면 그 줄을 안 붙이는가");
{
  ...
  /* ⚠️ 2026-09-03 에 열쇠의 **출처가 둘**이 됐다 — 사람이 짚은 `--kapt`, 그리고
     아침 판정기가 이름+지번을 서로 대조해 물려 로그에 남긴 `kaptCode`.
     둘 다 **이름 매칭이 아니다.** 금지된 것은 「카드 만들 때 이름만 보고 갖다 붙이는 것」이고
     (상록마을 2026-08-13), 판정이 이미 물린 값을 잇는 것은 붙이는 것이 아니라 **안 놓치는 것**이다.
     그래서 검사도 셋으로 나눠 판다 — 문구 하나를 grep 하면 표현만 바뀌어도 빨간불이 된다. */
