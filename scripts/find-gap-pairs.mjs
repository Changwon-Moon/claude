#!/usr/bin/env node
/**
 * 🔍 「같은 값에서 출발한 단지들」 묶음 찾기
 *
 *   node scripts/find-gap-pairs.mjs --base-max 10 --tol 3 --out gap-ep1   # 1편
 *   node scripts/find-gap-pairs.mjs --base-min 15 --tol 3 --out gap-ep2   # 2편
 *
 * ── 무엇을 하나 (오너 2026-09-02 확정)
 * 1,000세대 이상 명부 단지의 **전용 59·84 를 한 풀에 섞어** 놓고,
 * 기준창(2019-11~2020-03) 최고가가 **±TOL 안에서 겹치는 칸들**을 한 묶음으로 본다.
 * 그 다음 최근창(최근 6개월) 최고가의 **금액 차이(억)**로 줄을 세운다.
 * 카드 한 장에 2~3단지가 들어가고, 겹치지 않게 고를 때는 **단지** 단위로 잡는다.
 *
 * ── ⚠️ 기준가 폭은 좁을수록 카드가 **많이** 나온다 (2026-09-02 실측 · 반직관)
 *      ±3% → 1편 36장 · ±5% → 21장 · ±8% → 15장
 *    폭이 넓으면 한 묶음이 넓은 구간의 단지를 다 삼키고, 그 단지들이 쓰인 뒤로 다음 묶음이
 *    계속 걸러진다. 좁으면 묶음끼리 단지를 덜 뺏는다. 「같은 값이었다」는 주장도 더 강해져
 *    **두 축 모두에서 낫다** — 그래서 오너 기본은 `--tol 3` 이다.
 *
...
function main() {
  ...
  const meta = {
    ...
    /* 전수 검사가 묻는 칸 — 이 파일의 모든 수치는 1차 출처 캐시에서 **코드가** 계산한다.
       손으로 적은 값이 하나도 없다는 뜻이다. */
    verified: true,
