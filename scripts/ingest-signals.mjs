 ...
 *
 * 관제탑이 목표 비중을 갖고 있었는데 수집기가 몰랐다. **정해 둔 규칙을 코드가 모르면
 * 없는 규칙이다.** 그런데 그 비중이 네 곳에 있었고 이미 갈라져 있어서(관제탑은 '주식15',
 * 회고는 '증시·경제35') 2026-08-31 에 `data/topic-quota.json` 하나로 모았다.
 *
 * ⚠️ 막지 않고 **뒤로 미룬다.** 쿼터를 넘은 후보는 버리는 게 아니라 이번 회차에서만 빠진다 —
 *    다음 날 자리가 있으면 들어온다. 좋은 소재를 주제 때문에 영영 잃으면 안 된다. */
const QUOTA = (() => {
  /* 정본은 data/topic-quota.json 하나다 (2026-08-31). 여기 숫자를 다시 적지 않는다 —
     그전엔 목표 비중이 네 곳에 있었고 이미 갈라져 있었다. */
  const q = {};
  for (const t of JSON.parse(readFileSync(join(ROOT, "data/topic-quota.json"), "utf8")).topics) {
    q[t.key] = t.quota;
  }
  return q;
})();
