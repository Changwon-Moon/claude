...
/* ── 앞 N 줄: 카드 meta 그대로 */
/* ── 번호 — **몇 장이든 끊기지 않게** (2026-09-15 오너 "그동안 5개만 했었는데, 전체 추가해줘")
 *
 * 예전엔 다섯 칸짜리 배열이라 `--top` 을 6 이상 주면 번호가 `undefined` 로 나갔다.
 * 유니코드 키캡은 **0~9 와 🔟 까지만** 있다 — 11 부터는 그런 글자가 없다.
 * 그래서 열 장까지는 키캡, 그 뒤는 `11.` 꼴로 잇는다. 카톡은 둘 다 그대로 읽힌다.
 * ⚠️ 없는 글자를 만들어 내려고 「1️⃣1️⃣」처럼 두 개를 붙이지 않는다 — 11 이 아니라 1,1 로 읽힌다. */
const KEYCAP = ["1️⃣", "2️⃣", "3️⃣", "4️⃣", "5️⃣", "6️⃣", "7️⃣", "8️⃣", "9️⃣", "🔟"];
const NUM = new Proxy([], { get: (_, k) => (/^\d+$/.test(String(k)) ? (KEYCAP[Number(k)] ?? `${Number(k) + 1}.`) : undefined) });
