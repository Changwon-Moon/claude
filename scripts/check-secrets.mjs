 ...
 * 카카오 알림이 **조용히** 안 가고 있었다. 알림용 키는 등록된 적이 없는데,
 * 이름이 비슷한 다른 용도의 키(`KAKAO_REST_KEY` — 지오코딩용)가 있어 켜 둔 줄 알았다.
...
console.log("      (2026-07-31: 알림 키가 등록된 적 없이 몇 주가 지난 것을 이 대조로 발견했다)\n");
...
console.log("  · KAKAO_REST_KEY — 알림이 아니라 카카오 로컬 API(좌표) 용. Phase 3 예정분이라 아직 아무도 안 읽는다");
