...
const mapSvg = tohuhMapSvg({
  ...
  stamps: false,
  /* ── 종류별 색 축 — **매매는 빨강, 전세는 파랑** (오너 지시 2026-09-02)
   * 표 값·지도 그라데이션을 한 축으로 묶는다. 한쪽만 바꾸면 카드 안에 색이 둘 남는다.
   * 제목의 두 강조도 같은 축으로 맞바꾼다 — **금액이 그 카드의 색**이고 평형이 반대색이다
   * (아래 `title` 참조). 카드 하나에 색이 둘 남지 않게 하는 같은 규칙이다.
   *
  ...
  labelInk: true,
  /* 한강에 흰 케이싱 — **네 장 모두** 켠다(오너 2026-09-02).
   * 필요해진 건 파랑 지도지만(파랑 선 vs 파랑 면이라 색상 대비가 사라진다 — 실측 1.26:1),
   * 캐러셀 4장은 한 게시물이라 **같은 지도로 읽혀야** 한다. 빨강 2장만 옛 모양이면
   * 넘길 때 지도가 바뀐 것처럼 보인다. 빨강 쪽도 케이싱이 있어 나쁠 것이 없다. */
  riverCasing: true,
  /* 진한끝 대비(잉크 글자 기준): 매매 #E5484D 4.54 · 전세 #2E6BFF 3.94 — 둘 다 WCAG
   * 큰글자 AA(3.0) 위다. 계정이 이미 가진 두 색을 그대로 쓴다(새 색을 만들지 않는다). */
  ...(KIND === "mae"
    ? { colorLo: [255, 235, 232], colorHi: [229, 72, 77] }
    : { colorLo: [232, 238, 255], colorHi: [46, 107, 255] }),
...
const card = {
  ...
  /* 표 값 색 — 전세만 코발트로 옮긴다(매매는 판형 기본 빨강) */
  ...(KIND === "mae" ? {} : { accent: "cobalt" }),
  centerBody: true,
  ...
  /* 강조 둘 — 한 문장의 주어(어떤 집)와 술어(얼마)라 강조가 둘이어도 초점이 갈리지 않는다
   * (청약 카드의 「훅 + 금액 둘 다 코발트」와 같은 판단 — 청약분양-카드-기준 §2).
   *
   * ── 두 색을 **종류별로 맞바꾼다** (오너 2026-09-02)
   *   매매 : 평형 파랑(hi-b) · 금액 빨강(hi)
   *   전세 : 평형 빨강(hi)   · 금액 파랑(hi-b)
   * 이유는 **금액이 그 카드의 색을 입어야** 하기 때문이다. 표·지도가 매매는 빨강,
   * 전세는 파랑인데 제목의 금액만 둘 다 빨강이면, 전세 카드에서 제일 큰 글자만
   * 카드의 색 밖에 남는다. 금액을 카드 색으로 두고 평형을 반대색으로 돌리면
   * 두 강조의 대비는 그대로이고 카드는 한 색으로 읽힌다.
   * ⚠️ 색을 새로 만들지 않는다 — 쓰는 색은 계정의 빨강·코발트 그대로다. */
  title:
    KIND === "mae"
      ? `서울 <span class="hi-b">${PYEONG_NAME}</span> 평균 ${KIND_KO} <span class="hi">${seoul.eok.toFixed(1)}억</span>`
      : `서울 <span class="hi">${PYEONG_NAME}</span> 평균 ${KIND_KO} <span class="hi-b">${seoul.eok.toFixed(1)}억</span>`,
  fitTitle: true,
  unit: "억",
