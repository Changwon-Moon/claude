...
const mapSvg = tohuhMapSvg({
  ...
  textOf: (p) => won(p.v),
  /* "성남 수정 +15만" 은 신고가 카드의 "성남 수정 26" 보다 넓다 — 겹침 판정 폭을 키운다.
   * 168 로는 종로·성북, 마포·서대문이 스쳤다(렌더 확인) → 184. */
  labelWidth: 184,
});
...
/* 상위 몇 곳을 실을지는 **검수로 정했다**(designQa textclip).
 * 보조값(현재 월세)이 붙어 행이 2줄이 되면서 신고가 카드(16행)보다 적게 들어간다:
 *   16행 → 표가 하단 설명·푸터를 덮음 · 12행 → 2px 겹침 · **11행 → 통과**.
 * 40곳 전체는 캡션에 있다. */
const TOP = 11;
const MEDALS = ["🥇", "🥈", "🥉"];
...
const card = {
  ...
  note: `수도권 토지거래허가구역(서울25+경기15) · 아파트 평균 월세`,
  ...
  /* 오너 지시 문구. '전체' 같은 단정은 데이터가 허락할 때만 붙인다(아래 footnote 가 보완). */
  title: `최근 1년 토허제 40곳, <span class="hi">월세 상승 상황</span>`,
  /* 오너 지시: 제목 아래 부제(subtitle) 없음 */
  unit: "원", // 주값이 "+38만" → "+38만원". 기본값 '건' 은 신고가 카드용
  subUnit: "현재", // 보조값 앞에 붙는다 → "현재 227만원"
  head: { l: "지역", r: "1년간 오른 월세" },
  mapSvg,
  rows,
  /* 오너 지시 문구 그대로. 인과가 아니라 '대비'로만 쓴다. */
  /* 평균 금액은 그달 계약 구성에 흔들린다 — 이 한 줄을 빼면 오보가 된다 */
  footnote:
    `최근 1년 ${ymKo(BASE)}(${BASE_NOTE}) → ${ymKo(asOf)} · 아파트 평균 월세 기준 · ` +
    `동탄구는 화성시 전체 기준` +
