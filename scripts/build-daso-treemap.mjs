...
if (variants) {
  ...
  /* ── 캡션 — 형식의 정본은 docs/CAPTION.md §4 다.
   *   훅 한 줄(이모지) → 이모지 섹션 + 불릿 → 👉 takeaway → 📌 출처 → CTA → 해시태그(최대 5).
   * ⚠️ 처음에 이 규칙을 안 읽고 이모지 없이 스무 줄짜리를 썼다(오너 「장황하다」 2026-09-07).
   *   기준은 이미 있었고 내가 안 본 것이다 — 새 판형이라고 캡션까지 새로 지어내지 않는다.
   * 수치는 전부 위에서 계산한 값이다. 보고 옮겨 적지 않는다(§2). ── */
  const caption = [
    `🏠 집 가진 사람 100명 중, 여러 채 가진 사람은 몇 명일까요?`,
    ``,
    `법원 등기정보광장 기준 ${Math.round(multiSum)}명입니다.`,
    ``,
    `📊 집합건물 소유자 구성 (2026년 8월)`,
    `· 1채 ${oneShare.toFixed(2)}%`,
    `· 2채 이상 ${multiSum.toFixed(2)}% — 이 중 ${MULTI[0].v.toFixed(2)}%p가 딱 2채`,
    `· ${grouping.minTail}채 이상 ${grouping.tailItems.reduce((a, m) => a + m.v, 0).toFixed(2)}%`,
    ``,
    `👉 여러 채 가진 사람 열에 일곱은 두 채입니다.`,
    ``,
    `※ '집합건물'은 아파트·오피스텔·연립·다세대입니다.`,
    `   단독주택은 빠지고 오피스텔·상가는 들어갑니다.`,
    `※ 그래서 국가데이터처 주택소유통계의 다주택자 비율(2024년 14.9%)과는 다른 통계입니다.`,
    `📌 출처 · 법원 등기정보광장 「집합건물 소유지수·다소유지수」 (2026년 8월, 잠정)`,
    ``,
    `더 보기 👉 @wirit_note`,
    ``,
    /* 해시태그 상한 5개 — 정본은 rubric.ts 의 CAPTION_MAX_TAGS 다(초과는 error). */
    `#부동산통계 #다주택자 #집합건물 #오피스텔 #위릿`,
  ].join("\n");
