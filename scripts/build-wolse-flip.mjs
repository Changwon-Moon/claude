...
const caption = nl(
  ...
  /* 카드에서 산술 가정 한 줄이 빠졌으니(오너 지정 문구) 캡션에서 명시한다.
   * 추정 근거가 어디에도 안 적히면 추정치가 실측처럼 읽힌다. */
  `※ 카드의 ${lastYear}년 ${bare(estFull)}% 는 추정치입니다 —`,
  `   ${monthsDone}개월 실측 ${sign1(partialRow?.v ?? 0)} 에 × ${mulTxt} 한 값이며,`,
  `   남은 기간이 같은 속도로 오른다는 가정입니다(계절성·정책 변화 미반영).`,
