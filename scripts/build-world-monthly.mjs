...
/* 제목 — 수치는 **카드가 그린 값 그대로** 쓴다(사람이 옮겨 적지 않는다).
   BRAND: 제목 한 줄 안에서만 코발트+레드, 각 색 한 덩어리씩.
   코발트 = 무엇을 다루는 카드인지(주제) / 레드 = 그 크기(수·값)

   ⚠️ 1위가 바뀔 수 있는 제목이다. 누적을 **8월 말 기준으로 다시 세자 1·2위가 뒤집혔다**
      (오늘까지로 재면 대만이 앞선다). 그래서 제목의 나라 이름도 여기서 계산한다 —
      손으로 적어 두면 다음 달에 표와 제목이 갈린다.
   ⚠️ 조사(이/가·은/는)를 붙이는 자리를 만들지 않는다. 나라 이름이 바뀌면 받침이 바뀌고,
      「한국가 1등」 같은 문장이 조용히 나간다(첫 판에서 실제로 그렇게 나왔다). */
const title =
  `세계 1등은 <span class="ac">${top.country}</span>, ` +
  `${THROUGH}달 만에 <span class="hi">${sign1(top.ytdNum)}%</span>`;
...
const doc = {
  ...
  rows: rows.map(({ key, ytdNum, asOf, ...r }) => r),
