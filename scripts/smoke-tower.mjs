 ...
 * 탭 6종: 발행(기본) · 소재 · 보관함 · 회사 · 자산 · 배관.
 * (2026-08-31 「배관」 추가 — 예약 워크플로가 제때 왔는지 날짜별로 보는 계기판.
 *  탭이 늘어난 것 자체는 축소 원칙에 어긋나지 않는다. 이 탭은 **읽기 전용 관측**이고,
 *  축소가 막으려던 것은 "지시·제작 입구가 관제탑에 다시 생기는 것"이었다.)
 ...
 *
 * 검사하는 것:
 *   구조   탭 목록 · 지운 화면(결정함·칸반·지시함·성과)이 되살아나지 않았는가
...
console.log("\n[구조 — 탭 목록 + 지운 것이 되살아나지 않았는가]");
const tabs = await q(`[...document.querySelectorAll(".tab")].map(t=>t.dataset.v).join(",")`);
/* ⚠️ 목록을 통째로 못박지 않는다 — 2026-08-31 에 「배관」 탭을 더하자 이 줄이 떨어졌는데,
   막으려던 것(지시·제작 입구의 부활)과는 무관한 읽기 전용 관측 화면이었다.
   대신 **있어야 할 다섯이 이 순서로 앞에 있는가**를 본다. 새 관측 탭은 뒤에 붙는다. */
check("핵심 5탭이 순서대로 있다(발행·소재·보관함·회사·자산)",
  tabs.startsWith("publish,ideas,archive,company,assets"), tabs);
