...
if (existsSync(logDir)) {
  for (const f of readdirSync(logDir).sort().reverse()) {
     ...
     *   · 수지 풍덕천동 A44878309 10.8억 (09-02 판정)
     *   · 성동 마장동   A13381608 13.8억 (09-07 판정)
     *   · 노원 월계동   A13905105  9.6억 (09-08 판정)  ← 오늘 것
     * 하루치 원커맨드가 `--apt 현대 --kapt A13905105` 로 **오늘 것을 짚어 줬는데**,
     * 여기가 `--kapt` 를 안 보고 **이름만으로** 고른 뒤 「가장 비싼 건」 규칙을 적용해
     * **마장동 13.8억 카드**를 만들었다. 제목은 「현대」, 값은 남의 단지 것이다.
     * 하루 묶음에 안 들어간 것은 판정일 필터에 걸렸기 때문이지 **이 코드가 막은 게 아니다.**
     *
     * 이 저장소가 이미 아는 규칙이다 — 「이름으로 짚으면 남의 단지에 붙는다」(상록마을 2026-08-13).
     * 이름 사전(`rename-danji.mjs`)에는 그 규칙이 있었고 **여기엔 없었다.**
     *
     * ⚠️ 이건 **옛 카드를 지키는 일이기도 하다.** 로그는 매일 자란다. 신길동 삼환 84
     *    (08-25 확정·발행)는 그 뒤 09월에 뜬 **구운동 삼환 84** 가 이름만 같은 채로 들어와,
     *    다시 그리면 14.4억이 6.35억으로 **조용히 바뀌는 자리**였다. 이 걸러내기가 그걸 막는다.
     *
     * ⚠️ **「그 달에 그 코드가 없으면 이름으로 고른다」로 무르게 하면 안 된다.** 실제로 재 봤다:
     *    신길동 삼환 84 는 08월 로그에 있고 09월 로그에는 **구운동 삼환 84** 만 있다.
     *    무르게 하면 09월 파일에서 구운동을 집어 확정 카드가 6.35억으로 바뀐다.
     *    엄격히 거르면 09월에서 아무것도 안 나와 08월로 넘어가 **제 단지를 찾는다.**
     *
     * ⚠️ `--merge-blocks` 는 예외다. DMC센트럴자이처럼 1~4단지를 묶는 건은 로그의 kaptCode 가
     *    오너가 짚은 대표 코드와 다르다 — 여기서 거르면 확정된 카드가 아예 안 만들어진다.
     * ⚠️ kaptCode 가 **없는** 옛 건은 거르지 않는다 — 로그에 코드가 물리기 전 자료다.
     */
    if (KAPT_ARG && !MERGE) cand = cand.filter((h) => !h.kaptCode || h.kaptCode === KAPT_ARG);
    for (const h of cand) {
      {
        merged.push(h);
...
if (worst.atMaxIdx >= 0) {
  ...
  /* ── 🔀 **골 먼저, 시간 나중** (2026-09-11)
   *
   * 두 검사가 같은 상황을 다르게 끝냈다. 09-11 영흥숲푸르지오파크비엔(2023년 준공 · 10억 돌파)은
   * 첫 거래가 2023.08 이라 **겪은 사이클이 없다.** 최대 낙폭은 −8.3% 뿐인데,
   * 시간 검사(12개월)가 **먼저 던져서** 카드가 통째로 막혔다.
   * 낙폭 검사가 먼저 돌았다면 「지난 고점대비」 줄만 빼고 **카드는 나왔을 것**이다 —
   * 곡선도 신고가도 저점 대비도 다 멀쩡하다. 그게 오늘의 표지 카드였다.
   *
   * 둘 다 「사이클이라 부를 것이 없다」는 같은 말인데, 한쪽은 **우아한 답**을 갖고 있었다.
   * 그래서 순서를 바꾼다: **골이 얕으면 줄을 빼고 끝낸다**(시간 검사는 볼 필요가 없다).
   * 골이 깊은데 시간만 짧은 경우 — 진짜 급락·급반등 — 는 여전히 시간 검사가 던진다.
   *
   * ⚠️ `--accept-cycle-guard` 를 준 판은 **옛 순서 그대로**다. 오너가 그림을 보고
   *    「그려라」고 한 자리라, 여기서 줄을 빼 버리면 그 지시를 뒤집는 것이 된다.
   */
  ...
  if (Math.abs(worst.dd) < MIN_CYCLE_DROP && !flag("accept-cycle-guard")) {
    console.warn(
  ...
  /* 위 낙폭 검사가 이미 「사이클이라 부를 골이 없다」고 줄을 뺐으면 여기서 던질 것이 없다.
     같은 말을 두 번 하는 자리이고, 한쪽은 이미 우아하게 끝냈다(2026-09-11). */
  const CYCLE_ALIVE = worst.atMaxIdx >= 0;
  if (CYCLE_ALIVE && gap < MIN_CYCLE_MONTHS && flag("accept-cycle-guard")) {
  ...
  } else if (CYCLE_ALIVE && gap < MIN_CYCLE_MONTHS) {
    throw new Error(
...
if (KAPT) {
  ...
  if (stationKnownMissing) {
    ...
    /* 순서: **번호 노선 먼저, 숫자 작은 것부터** → 이름 노선(우이신설·신분당 …).
     * 카카오가 준 순서는 「출입구가 가까운 쪽」이라 카드마다 들쭉날쭉했다.
     * 사람들이 부르는 순서(「4호선/우이신설선 환승」)에 맞춘다(오너 2026-09-22). */
    const lineRank = (l) => (/^\d+$/.test(String(l)) ? Number(l) : 1000);
    const ordered = [...lines].sort((a, b) => lineRank(a) - lineRank(b));
    const badgeLines = ordered.slice(0, 2);
    if (ordered.length > badgeLines.length)
      console.log(`ⓘ 노선이 ${ordered.length}개라 뱃지에는 앞 2개만 그립니다: ${badgeLines.join("+")} (전체 ${ordered.join("+")})`);
    ...
    if (st.distanceM != null && st.distanceM > MAX_BADGE_M) {
      ...
      /* 알약이 둘이면 가로를 한 치수 더 줄인다 — `.sr-stn.duo` (2026-09-22) */
      station = { name: st.station, lines: badgeLines, distanceM: st.distanceM, compact, duo: badgeLines.length > 1 };
    }
  } else {
    console.warn(
