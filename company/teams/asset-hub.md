...
- 자동취득(로고 3티어·사진·라이선스 게이트): `docs/IMAGE_AUTOMATION.md`
...
| 07-26 | **완성물 색인은 자동 생성한다** — `scripts/build-archive.mjs`가 `data/review/sets.json`·캡션·검수 리포트·렌더 PNG 경로를 모아 소재 분류대로 묶어 `data/archive/index.json`에 쓴다. PNG·콘텐츠 JSON은 gitignore라 실물을 옮겨도 커밋되지 않는다 → **색인이 저장소에 남는 유일하게 정직한 정리**다 | 관제탑 보관함 |
**KPI**: 자산 재사용률, 카탈로그 무결성(CI 통과 유지) | **자동화**: 검증 R3, 수집 R0

## 학습 로그 — 2026-07-31 브랜드 로고 14종

- **자동 취득은 브랜드 로고에 약하다.** 국내 아파트 브랜드는 대개 모회사 사이트의 하위
  페이지라 독립 도메인·파비콘이 없다. Brandfetch 는 못 찾으면 **모회사 로고로 떨어지는데
  로그에는 성공으로 찍힌다.** 12종 중 실제로 맞은 것은 3종뿐이었다.
  → 브랜드 로고는 **오너에게 직접 받는 것이 정상 경로**다. 자동 취득은 회사 로고에만 기대한다.
- **한글 이름으로 슬러그를 만들 수 없다.** JS 의 `\w` 는 한글을 포함하지 않아 슬러그가 빈
  문자열이 되고, 기본값 'logo' 로 **여섯 개 파일이 서로를 덮어썼다.**
  → 도메인에서 슬러그를 만들고, 못 만들면 기본값 대신 **에러를 던진다.**
- **받은 파일은 규격이 제각각이다.** 여백이 0%인 것부터 94%인 것까지 있었다.
  들여올 때 잉크 범위로 자르고(`import-owner-logos.mjs`),
  누끼 + 크기 정규화를 돌린다(`normalize-logos.mjs`).
- **원본은 `_source/` 에 남긴다.** 정규화한 것을 또 정규화하면 계속 줄어든다.
  스크립트는 항상 `_source/` 를 입력으로 쓴다 — 두 번 돌려도 결과가 같다.
- **누끼는 문턱 하나로 자르지 않는다.** 흰색에 가까울수록 서서히 투명하게 해야 글자
  가장자리가 톱니로 남지 않는다.
- **파일을 새로 들여왔으면 정규화를 반드시 다시 돌린다.** 07-31 에 이 단계를 빠뜨려
  14개 중 11개가 원본 상태로 카드에 들어갔다.
