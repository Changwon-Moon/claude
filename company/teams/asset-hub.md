...
**한 줄**: 받기 전에 먼저 찾는다. 한 번 쓴 자료는 자산이다.

## ① 무엇을 보나 / 무엇을 안 보나

| 본다 | 안 본다 |
|---|---|
| 이미 있는 자산인가(카탈로그) | **어떻게 배치할까** → 디자인팀 |
| 라이선스·저작자 표기 | **수치 데이터** → 리서치팀 |
| 취득물이 **진짜 그것인가** | |
| 원본 보존(`_source/`) · 정규화 | |

> ⚠️ **자동 취득은 성공했다고 찍혀도 틀린 것을 가져온다.** Brandfetch 는 못 찾으면
> **모회사 로고로 떨어지면서 로그에는 성공으로 남는다** — 12종 중 3종만 맞았다.
> 아파트 브랜드 로고는 **오너에게 직접 받는 것이 정상 경로**다.

## ④ 최근 놓친 것

- **한글 이름으로 슬러그를 만들려다 6개 파일이 서로를 덮어썼다**(`\w` 는 한글을 모른다)
  → 못 만들면 기본값 대신 **에러를 던진다.**
- **들여온 뒤 정규화를 안 돌려 14개 중 11개가 원본 상태로 카드에 들어갔다**(07-31).
- **취득 로고를 눈으로 안 보고 썼다** — 삼성전자 대신 앱 아이콘이 붙었다.
