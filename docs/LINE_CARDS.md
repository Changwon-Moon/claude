...
- **단지명 줄바꿈은 단어(동네·건설사·브랜드) 단위** — `word-break: keep-all` + 빌더 DISP 의 ZWSP(빌더에서 `const Z="…"` 로 삽입). 음절 중간 끊김 금지. (`CARD_CHECKLIST.md §4` 참조)
- **전용면적 예외 표기(SUB)**: 84㎡가 없는 단지는 실제 전용면적으로 대체하고 단지명 아래 `전용 XXX㎡ 기준` 을 약하게 병기. 빌더 `SUB:{역:문구}` → 라이브러리가 `.slp-sub` 로 렌더(해당 템플릿에 `.slp-danjibox` 필요). 예: 5호선 목동=전용101·오목교=전용119·신금호=전용60, 6호선 이태원=전용106.
- **오너 지정 단지에 국토부 84㎡ 매매 실거래가 없으면**(신축·미거래) → 그 역 84㎡ 최고가 단지로 대체(오너 확인). 데이터셋 `meta.flags` 에 대체 사유를 남긴다. 대형/소형만 있으면 위 SUB 로 실전용 표기.
...
| 뱃지 | 반영 역(카드 기준) |
|---|---|
| GTX-A | 2호선 삼성 · 3호선 연신내·수서 · 6호선 연신내 |
| GTX-B | 5·9호선 여의도 · 7호선 상봉 · 8호선 별내 |
...
| SRT | 3호선 수서 |
| KTX | 7호선 상봉(중앙선 KTX-이음) |
| 동북선 | 2호선 왕십리 · 4호선 미아사거리 |
| 경춘선(경춘) | 6호선 신내 · 7호선 상봉 · 8호선 별내 |
| 공항철도(공항) | 5·6호선 공덕 · 6호선 DMC |
| 경의중앙(경의) | 5·6호선 공덕 · 6호선 DMC · 8호선 구리 · 7호선 상봉 등 |

- **GTX 뱃지는 축소 렌더**(font 11·폭·높이 작게) — 노선 레일에 안 겹치게(2026-08-02 오너 지시). `wirit-line.mjs badgeText` 가 `/^GTX/` 감지해 compact.
- **경춘·공항·경의 등 일반 환승 텍스트 뱃지**는 빌더 XFER 에 인라인(`["경춘","#0C8E72",0,0]` 등). EXP 맵은 예정·고속철도용.
- **뺀 것**: GTX-D(노선 미확정), 서부선·위례신사선(오너 지시로 제외), 강북횡단선(사업 보류).
- 한 역에 여러 개면 중앙 채널에 세로 스택(4개↑면 간격 축소, 예: 7호선 상봉=경의·경춘·GTX-B·KTX). 텍스트 뱃지는 compact pill.
...
> 2026-08-31 에 `docs/AGENTS.md` 부록에서 옮겼다(그 문서는 팀 문서 9개와 중복이라 폐기).
> 핵심은 한 줄이다 — **에이전트는 '어느 단지가 그 역 대장인가'만 판정하고,
> 84㎡ 최고가 숫자는 코드가 molit 캐시에서 다시 뽑는다.** 보고된 숫자는 믿지 않는다.
...
**이 그림이 알려 주는 것은 빠진 자리다** — 신설역 10개(도림사거리·대림삼거리·시흥사거리·목감·
장하·성포·호수·한양대·학온·매화)는 OSM 에 점이 없어 비어 보인다. 그 공백을 채울 좌표원을
정하기 전에는 카드로 올리지 않는다.

> 2026-09-08 에 이 줄을 적었다. 스파이크가 문서 없이 남아 있어 「고아 스크립트」로 잡혔는데,
> 지울 물건이 아니라 **아직 안 끝난 물건**이라 쓰는 법을 적어 살려 뒀다.
