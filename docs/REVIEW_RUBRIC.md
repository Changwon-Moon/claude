...
| # | 검수 항목 | 누가 재나 |
|---|---|---|
| 1 | 수치 = 원본 일치 (provenance 경로 대조) | 코드 |
| 2 | 순위·합계·단위 정합 (1위≥2위, % 합계 등) | 코드 |
| 3 | 출처 표기 존재·정확 | 코드 + LLM |
| 4 | 저작권: 사진·로고가 허브 등록 자산인가 | 코드 |
| 5 | 가독성: 넘침·겹침·저대비·워터마크 | `designQa` 좌표 실측 |
| 6 | 오해 소지: 제목-데이터 불일치, 과장, 상식 밖 수치 | 사람 눈 (+ LLM 있으면 보조) |

**6항 전부 통과여야 내보낸다.** 1·2·4·5 는 키 없이도 돌고, 3·6 은 사람이 본다.
