...
## 승인은 **한 번**이다 (2026-08-31 통합)

그전엔 오너가 같은 카드에 세 번 "예"를 눌러야 했다 —
`confirm.mjs`(채팅) → 관제탑 [🚀 발행 승인] → [✅ 올렸습니다].
앞의 둘은 **같은 판단**이었고, 실제로 아무도 두 번째를 안 눌렀다(확정 35 vs 승인 2).
관제탑이 `sets.json` 의 `state` 를 안 봐서 채팅에서 확정한 것이 화면에 안 떴기 때문이다.

지금은 **두 단계**다:
| 단계 | 누가 | 무엇 |
|---|---|---|
| ① 검토 중 | 세션이 만들고 오너가 본다 | 카드 실물 + 캡션 + **팀별 검수 결과** |
| ② 올릴 차례 | 채팅의 `confirm.mjs` **또는** 관제탑 버튼 | 둘 중 아무거나 한 번이면 된다 |
| ③ 발행됨 | 오너가 인스타에 올린 뒤 [✅ 올렸습니다] | `published/` 에 그날 실물이 보관된다 |

⚠️ 청약 카드는 **접수가 끝나면 「기한 지남」**이 붙는다(카드의 schedule 에서 읽는다).
자동으로 지우지는 않는다 — 내릴지는 오너 판단이다.
