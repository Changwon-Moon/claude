...
- 무순위(줍줍)도 **같은 판형**이다 — `kind: "remndr"` 로 칸의 뜻만 바뀐다.
  무순위 안에서 다시 판이 **둘**로 갈리고, 가르는 건 `price.byType` **유무**다(플래그를 손으로 켜지 않는다):
  없으면 *규모 우선*(전용면적별 잔여 세대 — 송도형), 있으면 *타입·분양가*(단지 규모 4칸 + 타입별 최고가 4칸 — 장위형).
  본청약 때 값이 매겨진 단지의 잔여 세대라면 분양가를 알고, 그러면 그게 소식이다.
- 조감도가 홈페이지 캡처라 단지가 작거나 치우쳤으면 **크롭으로 푼다 — 눈대중이 아니라 계산이다.**
  가로는 창문 격자 대비로 단지 무게중심을 재서 크롭 중심을 맞추고(크롭을 왼쪽으로 옮기면 단지는 오른쪽으로 간다),
  세로는 `shift = 남는 세로 × 42.5%` 를 뒤집어 푼다. 좌표는 `photo.provided` 에 적는다.
  → `docs/guides/청약분양-카드-기준.md` §3 「사진」
- **판형을 고칠 때는 새 CSS 를 선택자로 가둔다**(`.frow.four`, `:has(.frow.four)`).
  공통 규칙을 건드리면 확정본이 같이 움직인다 — 고친 뒤 확정본 md5 를 **전부** 다시 찍는다.
- 확정: `--publish` 로 `data/content/{날짜}/` 에 쓰고 → 캡션(반드시 '출처' 포함) → `builders.json`·
  `sets.json` 등록 → `produce-card.mjs` → `confirm.mjs`. **청약 카드는 정기물이라
  `pixel-baselines` 에 넣지 않는다.**
  확정본 3장: `danji-hangang`(`3dc78a13…`) · `danji-songdo`(`553d4532…`) · `danji-jangwi`(`5af157c8…`).
- **확정과 발행 승인은 다른 결정이다.** 픽셀을 고정해도 1차 출처 대조가 안 끝났으면
  `sets.json` 의 `state` 를 `발행 보류` 로 두고 `publishGate` 에 무엇이 남았는지 적는다.
  발행 대기열에는 올리지 않는다.
...
## 세션 종료 전 — **`wirit-close` 스킬로 넘긴다**

코워크 컨테이너는 세션과 함께 사라진다. **커밋하지 않은 작업은 없어진다.**
그리고 원천에 안 적은 배움은 다음 세션이 같은 지적을 다시 받게 만든다.

**이 스킬은 여는 쪽이다. 닫는 것은 `wirit-close` 스킬이 맡는다.**

> 🔒 마감 절차의 **정본은 저장소의 `docs/CARD_CHECKLIST.md` §7** 하나다.
> 예전엔 네 군데에 조금씩 다르게 적혀 있었고, 그래서 서명 순서 오류가 한 곳만
> 고쳐진 채 나머지는 계속 틀린 것을 가르쳤다(2026-08-16d). 절차를 바꾸면 **§7 만** 고친다.

```bash
node scripts/close-session.mjs   # 검사·학습 기록 확인·안 밀린 커밋까지 한 번에
```

꼭 기억할 것만:

- **먼저 쓰고 그 다음에 검사한다.** 뒤집으면 초록불에 안심해서 배운 것을 안 적고 닫는다
- 검사 순서는 **`tower-deploy.yml` 과 같아야** 하고 **서명은 맨 뒤**다 —
...
- **커밋 & 푸시** — 프록시 우회 옵션(`-c http.proxy= -c https.proxy=`)을 잊지 않는다.
  ...
  저장소가 **공개**이므로(2026-08-16 전환) 밀기 전에 비밀이 섞였는지 본다
- **조립 ≠ 배포.** 관제탑은 푸시가 방아쇠고, 올라갔는지는 Actions 기록으로 **본다**
