  ...
  for (const ym of months) {
    ...
    while (!done) {
      try {
        ...
        /* ⚠️ **문이 닫혔으면 여기서 접는다** (2026-08-26).
         *
         * 예전엔 5번(약 5분) 기다린 뒤에도 남은 달을 **하나씩 끝까지 두드렸다.** 그래서
         * 08-26 아침 실거래 API 장애 때 **한 단지가 60분 잡 타임아웃을 통째로 먹고**
         * 대기열 19줄 중 1줄에서 잘렸다 — 수집한 것은 0건이었다.
         * (그날 `molit.ts` 에 「연결 실패도 3번 재시도」를 넣은 직후라 한 달이 더 오래 걸렸다.
         *  재시도는 옳지만, **안 열리는 문 앞에서는 재시도가 시간을 태운다.**)
         *
         * `molitSingoCli` 는 2026-08-12 에 같은 것을 이미 배웠다 — 「5번 쉬고도 안 열리면
         * 접고 **왜 접었는지 말한다**」. 이쪽만 그 교훈이 빠져 있었다.
         *
         * 문이 닫힌 것은 **이 단지의 문제가 아니라 API 의 문제**다. 다음 줄로 넘어가 봐야
         * 똑같이 태운다 → **프로세스를 끝낸다.** 대기열은 남으므로 다시 밀면 이어서 간다. */
        if (doorWaits >= MAX_DOOR_WAITS) {
          console.error(
            `\n⛔ ${DOOR_WAIT_MS / 1000}초씩 ${MAX_DOOR_WAITS}번 기다렸는데도 안 열립니다 — 여기서 접습니다.\n` +
              `   (남은 ${months.length - points.length}개월을 계속 두드리면 잡 시간만 태우고 결과는 같습니다)\n` +
