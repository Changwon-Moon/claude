...
/* ── 청약홈 APT 주택형별 분양정보(15101047, 파일데이터 자동변환) 응답 모사 ─────────
 * ⚠️ **키 이름은 유추다.** 포털 항목표에 한글 이름만 있고 영문명 칸은 비어 있다
 * (2026-08-28 확인). 자동변환 API 가 한글 키를 그대로 낼지는 응답을 봐야 안다 —
 * 분양권 수집기에서 `dealTypeNm` 유추가 틀렸던 그 자리다. 그래서 파서는 후보를
 * 나열하고, 첫 수집이 `data/bunyang-price-probe.md` 에 키 목록과 원본 한 행을 남긴다.
 * 여기서 못박는 것은 **주택형에서 전용면적을 뽑는 규칙**과 **애매하면 안 잇는다**이다. */
export const BUNYANG_PRICE_JSON = JSON.stringify({
  currentCount: 4,
  page: 1,
  perPage: 1000,
  totalCount: 4,
