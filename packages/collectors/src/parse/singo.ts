...
export function alertBody(hits: SingoHit[], topN = 0, today?: string): string[] {
  ...
  /* ── 이 목록이 **무엇의 하루치인지**를 목록 스스로 말하게 한다 (2026-08-24 오너)
   *
   * 오너: *"수집 및 메시지 기준을 매일 1일동안 있었던 신고가 거래로 변경하려고 해."*
   * 그런데 **이미 그렇게 돌고 있었다** — 알림에 실리는 것은 그날 처음 문턱을 넘은 건뿐이다.
   * 오해가 난 이유는 목록의 **계약일이 08.03~08.15 처럼 흩어져 보여서**다.
   * 실거래 신고기한이 계약 후 30일이라 그렇다(8월 히트 93건의 계약→확인 중앙값 **10일**,
   * 계약 다음 날 들어온 건 **0건**). 즉 계약일로 하루를 자르면 알림은 매일 텅 빈다.
   *
   * 문구가 그 사실을 안 적고 있었으니 읽는 사람이 매번 되물을 수밖에 없다.
   * **재는 것과 말하는 것이 같아야 한다.** 기준을 바꾼 게 아니라 기준을 적었다. */
  //
  // ⚠️ **굵게 표시를 쓰지 않는다.** notify-telegram 은 `parse_mode` 를 안 넘겨 **평문**으로 보낸다 —
  //    `**...**` 를 적으면 별표가 그대로 찍힌다. 알림 문구에는 마크다운을 섞지 않는다.
  lines.push("");
  lines.push("※ 계약일 기준이 아니라 「오늘 새로 드러난」 건입니다.");
