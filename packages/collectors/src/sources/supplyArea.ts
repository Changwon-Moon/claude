...
export type SupplyArea = {
  ...
  /** 전용률이 흔한 범위(70~90%) 밖이면 경고를 남긴다 — 막지는 않는다(사람이 본다) */
  warn?: string;
};

export const PY_M2 = 3.305785;

/* ── 📏 **전용률이 「흔한 범위」인가** — 이 값이 문지기의 문턱이다
 *
 * 처음엔 70~85% 였다. 그런데 **위쪽 문턱이 너무 좁았다**:
 * 09-10 가산두산위브 85.3% · 09-11 용산 리버힐삼성 85.3% — 이틀 연속으로 걸렸고
 * 둘 다 들여다보니 **주차·대피 섞임이 아니라 그냥 효율 좋은 단지**였다
 * (지하대피소는 이미 빠져 있었고 남은 주거공용은 계단·승강기 하나뿐이었다).
 * 그때마다 오너가 평수를 확인해 통과시켜야 했다 — 문지기가 사람을 부르는 횟수가
 * 실제 위험보다 많으면, 사람은 결국 보지 않고 통과시키게 된다. 그게 문지기가 죽는 방식이다.
 *
 * 받아 둔 807건을 실측해 고쳤다(2026-09-11 · 오너 "문턱은 90%를 넘지 않도록"):
 *   · 70~85% … 718건   · **85~90% … 45건**   · 90% 초과 … **2건**
 * 위 문턱을 90% 로 올리면 **부르는 횟수가 45건 줄고, 정말 이상한 2건은 그대로 걸린다.**
 *
 * ⚠️ 아래쪽 70% 는 **그대로 둔다.** 그쪽은 주차장·대피소가 주거공용에 섞이는
 *    실제 사고 자리다(석수e-편한세상 59.6% → 30평 · 2026-09-05).
 */
export const RATIO_LO = 0.70;
export const RATIO_HI = 0.90;
...
export function supplyAreaOf(rows: Row[], wantExclusive: number, tolerance = 0.6): SupplyArea | null {
  ...
  const warn = ratio < RATIO_LO || ratio > RATIO_HI
    ? `전용률 ${(ratio * 100).toFixed(1)}% — 흔한 범위(${RATIO_LO * 100}~${RATIO_HI * 100}%) 밖이다. `
