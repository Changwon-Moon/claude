...
export const TABLES: Record<string, TableSpec> = {
  ...
  deaths: {
    ...
    note: "probe 로 축 전부 확인(2026-08-03): objL1=사망원인 52개(**0=계**) · objL2=행정구역 · " +
      ...
      "코드 체계 검증 완료: 지역 목록 385개를 받아 이름으로 묶었다(367개 중 348개, 94.8%). " +
      "미매칭 19개는 폐지된 행정구역(계룡시출장소·통합창원시·마산시·북제주군…).\n" +
      "⚠️ **시도 이름이 표마다 다르다**: 사망표는 '강원도·전라북도·제주도' 인데 " +
      "출생표·인구표는 '강원특별자치도·전북특별자치도·제주특별자치도' 다. " +
  ...
  /* ── 주택소유통계(국가데이터처 orgId 101) — 2026-09-09 신설, 전부 미검증 ──
     소재: ① 무주택/1주택/다주택 가구 구성 ② 시도별 주택소유율 지도
           ③ 「다주택자 비율이 기사마다 다른 이유」 — 세는 단위가 다르다.
     ⚠️ 이 통계는 **등기정보광장 다소유지수와 다른 물건이다.**
        · 다소유지수 = 집합건물 소유'자' 중 2채 이상 (오피스텔·상가 포함, 단독주택 제외)
        · 이 통계   = '주택' 기준. 가구 단위와 개인 단위가 따로 있다.
        같은 '다주택'이라는 말이 붙지만 모집단이 셋 다 다르다 — 카드에서 나란히 놓을 때
        반드시 단위를 적는다(2026-09-07 오너가 11% / 16% 를 두 번 혼동했다).
     ⚠️ 축·항목코드는 **아직 아무것도 확인 안 됐다.** probe 결과를 보고 채운다.
        특히 소유 주택수 구간(1건/2건/…)이 C1 인지 C2 인지, 무주택 가구가 이 표에 있는지
        (소유가구만 있는 표일 수 있다) 를 먼저 본다. */
  houseOwnHh: {
    orgId: "101", tblId: "DT_1LC0033", label: "주택소유별 가구",
    metric: "주택소유가구", itmId: "T1", objL1: "ALL", objL2: "", prdSe: "Y",
    confidence: "표명확실", enabled: false,
    note: "미검증(2026-09-09 등록). 전국 단위 소유 주택수별 가구. " +
      "보도자료(2024년 기준, 2025-11-14 공표) 대조값: 일반가구 2,229.4만 · 주택소유가구 1,268.4만 " +
      "(소유율 56.9%) · 무주택 961.0만. probe 로 축과 항목을 확인하기 전에는 카드에 쓰지 않는다.",
  },
  houseOwnHhSido: {
    orgId: "101", tblId: "DT_1LC0034", label: "주택소유별 가구_시도",
    metric: "주택소유가구", itmId: "T1", objL1: "ALL", objL2: "", prdSe: "Y",
    confidence: "표명확실", enabled: false,
    note: "미검증(2026-09-09 등록). 시도별 주택소유율 지도(②안)의 원자료. " +
      "보도자료 대조값: 소유율 최고 울산 64.0% · 최저 서울 48.1%. " +
      "⚠️ 지도에 붙이려면 data/geo/kosis-region-map.json 대조표를 거친다 — " +
      "표마다 행정구역 코드 체계가 달라 숫자로 조인하면 조용히 어긋난다(사망표 사례).",
  },

  /* ── 억대 연봉 근로소득자 카드 (2026-09-14 등록) ──
     기사(머니투데이 2026-09-14)가 인용한 국세청 원자료를 우리가 직접 세기 위한 표.
     ⚠️ 기준이 둘이라 섞으면 그대로 오보다:
        · 총급여액          = 연말정산 '총급여' (기사·보도자료가 쓰는 '연봉 1억')
        · 과세대상 근로소득 = 비과세를 뺀 값. 4.2.11·4.2.12 가 이 기준이다 — 같은 '1억'이 아니다.
     대조값(2차 출처): 2023년 억대 139만명·6.7% · 신고인원 2,085만명 (한국세정신문 2024)
                      2024년 억대 154만5,725명·7.3% (머니투데이 2026-09-14, 자료 국세청)
     probe 로 총급여액 규모 구간이 어느 축인지·인원 항목코드가 무엇인지 확인 전까지 카드에 안 쓴다. */
  wageYearEnd: {
    orgId: "133", tblId: "TX_13301_A035", label: "4.2.1 근로소득 연말정산 신고 현황Ⅰ(총괄)[2005~]",
    metric: "근로소득신고인원", itmId: "16133AAC5", objL1: "15133SGI03", objL2: "15133SGJ0B",
    prdSe: "Y", confidence: "확실", enabled: false,
    extraObjL: ["15133SGJ0B"],
    note: "probe 확인(2026-09-14). ⚠️ 이 표는 **규모별이 아니라 신고항목별**이다 — 1억 초과 인원은 " +
      "여기 없다(그건 wageBracket). 여기서 뽑는 것은 **전체 연말정산 신고인원**(비중의 분모)뿐이다. " +
      "축: C1=신고항목별(15133SGI03=과세대상 근로소득(총급여)) · C2=15133SGJ0B(연말정산신고자). " +
      "항목: 16133AAC5=인원 · 16133AAA1=금액. 최신 시점 2024. " +
      "⚠️ C2 에 '급여총계 상위 10%'(15133SGJ0C)가 있다 — 이걸 집으면 분모가 10분의 1이 된다.",
  },

  /* 1억 초과 근로소득자 인원의 원자료. 4.2.11 은 주소지(시도) × 과세대상 근로소득 규모별이다.
     ⚠️ 국세청은 '총급여'를 '과세대상 근로소득'이라고 부른다(4.2.1 의 C1 라벨이
        "15133SGI03=과세대상 근로소득(총급여)" 로 둘을 같이 적어 준다). 다른 기준이 아니다. */
  wageBracket: {
    orgId: "133", tblId: "DT_133N_427",
    label: "4.2.11 주소지별·과세대상 근로소득 규모별 연말정산 신고 현황[2007~]",
    metric: "근로소득자", itmId: "16133T2008_0135", objL1: "ALL", objL2: "15133JSJ00",
    prdSe: "Y", confidence: "확실", enabled: false,
    regionAxis: "C2",
    extraObjL: ["15133JSJ00"],
    note: "probe 확인(2026-09-14). C1=과세대상근로소득(총급여)규모별 · C2=주소지(15133JSJ00=합계) · " +
      "항목 16133T2008_0135=인원. 최신 시점 2024. " +
      "⚠️ **1억 초과는 한 칸이 아니다** — 구간이 '1억 이하'까지만 있고 그 위는 2억·3억·5억·10억 이하 + " +
      "10억 초과로 쪼개져 있다. 다섯 칸을 더해야 하고, 한 칸만 집으면 나오는 숫자도 그럴듯해 보인다. " +
      "⚠️ 같은 표에 금액(16133T2008_0317)·과세표준·결정세액이 있다 — 인원과 섞지 않는다. " +
      "수집기는 wageBracketCli.ts(정기 수집에는 안 낀다 — 연 1회짜리라 전용 워크플로가 부른다).",
  },

  /* 연도별 전국 주민등록 총인구. population(DT_1B040A3) 과 같은 표를 연간·전국으로 부른다.
     ⚠️ 월간 spec 을 그대로 쓰면 안 된다 — objL1:"ALL" 은 시군구 277개라 연간 16년이면 셀이 터지고,
        카드가 필요한 것은 전국 한 줄뿐이다. */
  populationYear: {
    orgId: "101", tblId: "DT_1B040A3", label: "행정구역(시군구)별 성별 인구수 — 연간·전국",
    metric: "총인구", itmId: "T20", objL1: "00", objL2: "", prdSe: "Y",
    confidence: "확실", enabled: false,
    note: "probe 확인(2026-09-14). 연간·전국 한 줄이 온다 — C1 축에 '00=전국' 하나뿐이고 " +
      "T20=총인구수, 단위 명. 2025년 51,117,378명이 왔고 이는 e-나라지표 공표값과 같다. " +
      "⚠️ 정기 수집에는 안 낀다(enabled:false) — 카드가 필요할 때만 부르는 연 1회짜리다. " +
      "월간 population 과 **같은 표**이므로 두 값을 한 카드에 섞지 않는다(연말 기준 vs 월말 기준).",
  },
};
