   ...
   ────────────────────────────────────────────────────────── */
{
  ...
  /* ── 📐 같은 전용면적끼리 묶는다 (오너 2026-09-08) — 그랑시티자이2차에서 드러난 자리.
     한 「59타입」에 전용 59.9068 과 59.9595 가 같이 있으면 예전엔 한 줄로 접혀 비싼 쪽만
     남았다. 판정은 면적 단위로 보는데 곡선은 타입 단위라 카드에서 어긋났다. */
  const two = mcFold(
    [
      T({ aptNm: "그랑시티자이2차", umdNm: "사동", area: 59.9068, priceManwon: 57000 }),
      T({ aptNm: "그랑시티자이2차", umdNm: "사동", area: 59.9595, priceManwon: 56000 }),
      T({ aptNm: "그랑시티자이2차", umdNm: "사동", area: 59.9595, priceManwon: 55000 }),
    ],
    yes,
  );
  const eq = (a: string, b: string) => a === b;
  check("전용면적이 다르면 다른 줄로 접는다", two.length === 2, `줄 ${two.length}개`);
  check("면적을 짚으면 **그 면적의** 최고가를 준다",
    mcPick(two, "그랑시티자이2차", "59", "사동", eq, 59.9595)?.max === 56000, "");
  check("면적을 짚으면 그 면적의 건수만 센다",
    mcPick(two, "그랑시티자이2차", "59", "사동", eq, 59.9595)?.count === 2, "");
  check("면적을 안 짚으면 **예전 그대로** 타입 최고가 — 확정된 곡선이 안 흔들린다",
    mcPick(two, "그랑시티자이2차", "59", "사동", eq)?.max === 57000, "");
  check("면적을 안 짚으면 건수도 예전 그대로(합산)",
    mcPick(two, "그랑시티자이2차", "59", "사동", eq)?.count === 3, "");
  check("소수점 흔들림은 같은 면적으로 본다(오차 0.01)",
    mcPick(two, "그랑시티자이2차", "59", "사동", eq, 59.96)?.max === 56000, "");
  check("없는 면적을 짚으면 null — 지어내지 않는다",
    mcPick(two, "그랑시티자이2차", "59", "사동", eq, 84.9) === null, "");
